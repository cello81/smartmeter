...
/* Sommer:*/    $mde->SetJSTimestamp($time->getTimestamp()*1000 + (2*60*60*1000)); // convert from php to js, add 2 hours
     ...
     /**
     * @Route("/detailrange/{datetoshowfrom}/{datetoshowto}/")
     */

    public function PVDataAction($datetoshowfrom, $datetoshowto)
    {
        $events = DetailController::GetEvents($datetoshowfrom, $datetoshowto);
	$pvdata = array();

	$pvdata['events']   = $events;
	$pvdata['consume']  = DetailController::ShowConsume();
	$pvdata['deliver']  = DetailController::ShowDeliver();
	$pvdata['site']     = DetailController::ShowSite();
	$pvdata['siteOst']     = DetailController::ShowSiteOst();
	$pvdata['siteWest']     = DetailController::ShowSiteWest();
	$pvdata['receive']  = DetailController::ShowReceive();
//	$pvdata['costs']    = DetailController::ShowCosts($datetoshowfrom, $datetoshowto);
     ...
     /**
     * @Route("/detail", name="_detaildashboard")
     */
	...
	return DetailController::PVDataAction($todayMorningString,'NOW');
    }
...
//echo $time->format('Y:m:d H:i:s');
	...
	for ($i = 1; $i <= 12; $i++ )
		...
		$dayDataOfMonth = DetailController::GetMonthData($monthString);
     ...
     /**
     * @Route("/pvdetail/month/{month}", name="_monthselect")
     */
    public function PVMonthAction($month)
    {
	$monthdata = DetailController::GetMonthData($month);
     ...
     /**
     * @Route("/pvdetail/select/{year}", name="_detailyearselect")
     */
    public function PVYearAction($year)
    {
	$yeardata = DetailController::GetYearData($year);
        return $this->render(
                'pv/year.html.twig',
     ...
     /**
     * @Route("/pv/detailmonth", name="_detailmonth")
     */
    public function ShowPVMonth()
    {
	$today = new DateTime('today');
	$actualMonth = $today->format("Y-m"); // 2016-11
	return DetailController::PVMonthAction($actualMonth);
    }

     /**
     * @Route("/pv/detailyear", name="_detailyear")
     */
    public function ShowPVYear()
    {
	$today = new DateTime('today');
	$actualYear = $today->format("Y"); // 2016
	return DetailController::PVYearAction($actualYear);
    }

     /**
     * @Route("/show/detailcost/")
     */
